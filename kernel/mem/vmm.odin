...
`space_destroy` frees the whole tree at once instead, which is where a
process's tables were always going to go.

**It had no caller for a long time**, and `segbrk` is what finally wanted one:
a run that shrinks has to stop being reachable *before* its frames go back to
the allocator, or ring 3 keeps a mapping to memory somebody else is about to
own. A page that was never mapped answers false, which is the state the caller
