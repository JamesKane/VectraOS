...
directory read shows `ls` twice, while *opening* `/bin/ls` gets the first.
Filtering would mean holding every name already emitted for the life of the
read -- unbounded in the size of the directory, in the kernel, on behalf of a
...
offset must be zero or a value previously returned in an Rreaddir entry, never
...
`c` must be open. Zero bytes with no error is the end of the directory --
including the end of the last member of a union, which is the only end a caller
ever sees.
...
readdir_one :: proc(c: ^Chan, offset: u64, buf: []u8) -> (n: int, err: Errno) {
	...
	// The server was told how much room there is, so a payload that does not
	// fit is a server bug. Truncating would cut an entry in half and hand the
	// caller a cursor that fails mid-name; refusing says which side is wrong.
...
the property that matters here: the cookie is absolute, so nothing is carried
between calls and a caller may abandon a listing halfway through without
leaking a fid on every tree it touched. When there is a process to hang the
state off, this is where a cached per-member fid goes.

Rebinding a union while a listing is in progress is undefined -- the member
index in a cookie names a position in a list that has changed. Plan 9 has the
...
readdir_union :: proc(
	...
	for ; idx < MAX_UNION_MEMBERS; idx += 1 {
		...
		substituted, so that member is read through it rather than opened a
		second time. Matched by (server, qid.path) rather than assumed to be
		member zero: the assumption is true today and is exactly the kind that
		stops being true quietly, leaving a listing that reads one member's
		entries under another member's index.
		*/
		src := c
		borrowed := true
...
change, and the re-encode is exact: only `offset` differs, so the payload is
the same length it arrived as. That is what lets the request ask for exactly
the room left in `out` and be sure the answer fits.
*/
@(private)
union_pass :: proc(
	src: ^Chan,
