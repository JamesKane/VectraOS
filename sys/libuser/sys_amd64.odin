...
/*
The door, and the instruction written as its two bytes.
...
One door of six arguments, and `sys.odin` makes the narrower five out of
it: a register the call does not read costs a zero and nothing else. This
file is the amd64 half of `sys.odin`. A port supplies its own door under
its own suffix and changes nothing above it.
*/
@(private)
